...
public class DrawGuitarNotes {
	/**
	 * Group Object to store drawings
	 */
...
/**
 * Method to check if a note has a slur on it 
 * @param note
 * @return True if note has slur
 */
	...
	/**
	 *  Method to check if a note  is a grace note
	 * @param note
	 * 	
	 * @return True if note is grace note
	 */
	private boolean hasGrace(Note note) {
...
/**
 * Method to add grace note to the drawing
 * 
...
//				Check if fret is double digit and set effect
	...
	/**
	 * Method to add a top slur to a fret 
	 * 
	 * @param horizontalPosition
	 * @param stringPosition
	 * @param rowIndex
...
/**
 * Method to add a bottom slur to a fret 
 * 
 * @param horizontalPosition
 * @param stringPosition
 * @param rowIndex
 ...
 */
	...
	/**
	 * Method to draw a slur from the direction specified by the note
	 * 
	 * @param horizontalPosition
	 * @param stringPosition
	 * @param rowIndex
	...
	private void drawSlur(int horizontalPosition, int stringPosition, int rowIndex, int fret, Note note) {
		if (note.getNotations().getSlurs().get(0).getType().equals("start")
				&& note.getNotations().getSlurs().get(0).getPlacement() != null) {
			topSlur(horizontalPosition, stringPosition, rowIndex, fret);
		} else if (note.getNotations().getSlurs().get(0).getType().equals("stop")
				&& note.getNotations().getSlurs().get(0).getPlacement() == null) {
			topSlur(horizontalPosition, stringPosition, rowIndex, fret);
		} else {
...
//			Draw Slurs
			...
			if (hasGrace(entry.getValue().get(0))){
//				draw grace notes
